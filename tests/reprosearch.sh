...
do
  ...
  # Only info lines carry a node report. expect echoes the `go nodes N` command
  # itself, and counting that echo let a run where the engine reported nothing
  # pass the even-count test on the echoes alone.
  #
  # pipefail is on, so a dead expect or a grep that matched nothing fails the
  # assignment rather than the emptiness check below. Catch it into an empty
  # `counts` so the round is reported by its cause and not by a line number.
  counts=$(expect repeat.exp $nodes 2>&1 | grep '^info ' | grep -o "nodes [0-9]*" \
             | sort | uniq -c) || counts=""
