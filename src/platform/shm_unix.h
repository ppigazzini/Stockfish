...
// -fno-exceptions and the only construction site is inside a `noexcept`
// member, so that throw is std::terminate, taken BEFORE open() can return
// false and let the fallback do its job. pthread_create reports the same
// condition as an errno.
//
...
class SharedMemory {
   ...
   private:
    ...
    static auto
    make_server_body(UniqueFd fd, UniqueFd shutdown_receiver, UniqueFd server_fd) {
