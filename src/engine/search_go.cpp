...
//
// Worker 0 gets the SearchManager and the others a null one, matching the pool.
// Index 0 is what is_mainthread() tests, and only the main worker applies the
// depth cap and drives the aspiration loop.
void build_workers(Context& c, usize n) {
    ...
    for (usize i = 0; i < n; ++i)
        c.workers.push_back(make_arena_unique<Worker>(
          *c.shared,
          i == 0 ? std::make_unique<SearchManager>(c.updates) : nullptr,
