...
struct TTEntry {
    ...
    // than a member read because the only caller has already asked the entry for
    // it -- probe() needs the same byte to decide occupancy -- and a relaxed
    // atomic named twice is two loads the compiler may not fold into one.
