...
//
//     e = lmrScale[i] * lmr_divisor(i + 1) - 2^40
//
...
// that is a power of two, and no divisor the closed form produces is one today
// -- but that is a property of the tuned numbers, not of the construction, so
// it is checked here.
static_assert([] {
    for (usize i = 0; i < lmrScale.size(); ++i)
        if (i64(lmrScale[i]) * lmr_divisor(int(i) + 1) - (i64(1) << 40) <= 0)
            return false;
    return true;
}(), "a lmr_divisor value divides 2^40, so the lmrScale correction overshoots");
...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const int   wnpcv  = shared.nonpawn_correction<WHITE>(pos, us);
    const int   bnpcv  = shared.nonpawn_correction<BLACK>(pos, us);
    const int   cntcv =
      m.is_ok()
          ? 7885
...
void update_correction_history(const Position& pos,
    ...
    shared.pawn_correction(pos, us) << bonus;
    shared.minor_piece_correction(pos, us) << bonus * 150 / 128;
    shared.nonpawn_correction<WHITE>(pos, us) << bonus * nonPawnWeight / 128;
    shared.nonpawn_correction<BLACK>(pos, us) << bonus * nonPawnWeight / 128;
...
bool Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth >= limits.depth))
    {
        ...
        const bool abortedLossSearch = stopFlag.load() && !pvIdx && rootMoves[0].is_exact_loss();
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, const bool cutNode) {
        ...
        && ttData.depth > depth - (ttData.value <= beta))
    {
        ...
        if (covers(ttData.bound, ttData.value >= beta)
        ...
        else if (ttData.bound != BOUND_EXACT && covers(ttData.bound, ttData.value < beta)
                 && depth > 5)
        {
