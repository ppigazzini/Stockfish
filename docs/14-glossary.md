...
| **continuation history** | a plane per (piece, destination) of the *previous* move, each plane a history over the current move. `ContinuationHistory` in `src/engine/history.h`, held in a `ContinuationHistoryBlock` (`src/engine/history.h`) whose `[2][2]` table is reachable only through `operator()(InCheck, Capture)` -- both subscripts are one-bit scoped enums, so `[capture][inCheck]` has no spelling. A call site reads `continuationHistory(InCheck(ss->inCheck), Capture(capture))`, and that name is two different members: `SharedHistories::continuationHistory()` returns the block, `Search::Worker::continuationHistory` is a reference to it, and `Stack::continuationHistory` is a `PieceToHistory*` into one plane |
...
| **psqt and positional** | the network's two output heads. `Network::evaluate` (`src/engine/nnue/network.cpp`) returns only their sum, the raw evaluation, so nothing outside the network reads either head alone |
| **optimism** | the per-worker search disposition blended into the evaluation, one of the three reasons Lazy SMP threads diverge. Set per root move in `Search::Worker::iterative_deepening` and read by `Search::Worker::evaluate`, both `src/engine/search.cpp`; `scale_evaluation` (`src/engine/evaluate.cpp`) weights it by the alignment between the network and the material balance |
