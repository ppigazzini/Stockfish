...
`Search::Worker::do_move` prefetches three more, and they are a different set: the
continuation-correction entries the *child* will read, taken from the parent's `(ss - 1)`,
`(ss - 3)` and `(ss - 5)`. The child reads `(ss - 2)`, `(ss - 4)` and `(ss - 6)` in
`correction_value`, and child `ss` is parent `ss + 1`, so the addresses match exactly for a normal
move -- castling and promotion prefetch an approximation and land on an unused line. Keep the three
offsets in step with the reads in `correction_value`: shifting one without the others costs the
prefetch silently, because a prefetch to the wrong line is not a fault and no gate can see it.
...
|---|---|---|
| `tests/reprosearch.sh` | `ucinewgame` resets everything a search reads | this page |
| `tests/fuzzsearch.sh` | the node body survives positions no bench list and no golden corpus contains, under ASan and UBSan, with no host registered | this page |
...
A different instrument from `tests/fuzz.py`'s harnesses ([10-tooling-ci.md](10-tooling-ci.md)), not a fourth one of them. Those drive the
